...
module Fixtures =
    ...
    let getFixtureCount (competitionId: Uuid, stage: string) = task {
    ...
    let getQualifiedTeams (competitionId: Uuid) = task {
        let! teams =
...
module Predictions =
    ...
    let ofFixture (competitionId: Uuid) (externalFixtureId: int64) = task {
        let pipelines =
    ...
    let ofStage (competitionId: Uuid, stage: string) =
        match stage with
        | "ROUND_OF_16"
        | "QUARTER_FINALS"
        | "SEMI_FINALS" ->
    ...
    let private idToUuid (competitionId, email) =
    ...
    let get id = task {
        let idFilter = Builders.Filter.Eq((fun x -> x.Id), (idToUuid id))
    ...
    let find (competitionId: Uuid) (term: string) = task {
        let filter =
            Builders.Filter.And(
    ...
    let updateQualifiers (competitionId: Uuid, stage: string, teamId: int64, hasQualified: bool) = task {
    ...
    let setUnqualifiedTeams (competitionId: Uuid, teams: int64 array) = task {
    ...
    let getScoreTable (competitionId: Uuid) = task {
